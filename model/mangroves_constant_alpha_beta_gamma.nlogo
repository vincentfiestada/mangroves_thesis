...
"" ""
PENS
"Natives" 1.0 0 -12087248 true "" "plotxy years count mangroves with [species = \"native\"]"
"Storms" 1.0 2 -2674135 true "" "if stormOccurred = True [\n    plot-pen-up\n    plot-pen-down\n    plotxy ticks plot-y-max\n  ]"
"Planteds" 1.0 0 -14730904 true "" "plotxy years count mangroves with [species = \"planted\"]"
...
@#$#@#$#@
<experiments>
  <experiment name="Monte Carlo (Vary Correlation Time, Diffusion Rate, and Range Offset" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>simulate</go>
    <metric>count turtles with [species = "native"]</metric>
    <metric>count turtles with [species = "planted"]</metric>
    <metric>avgRegTimeNative / (max list regTimeNativeCount 1)</metric>
    <metric>avgRegTimePlanted / (max list regTimePlantedCount 1)</metric>
    <metric>avgLifespan / (max list deaths 1)</metric>
    <steppedValueSet variable="correlation-time" first="0.25" step="0.25" last="2"/>
    ...
    <steppedValueSet variable="diffusion-rate" first="0.25" step="0.25" last="2"/>
  ...
  <experiment name="Monte Carlo Continued" repetitions="5" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>simulate</go>
    <metric>count turtles with [species = "native"]</metric>
    <metric>count turtles with [species = "planted"]</metric>
    ...
    <metric>avgRegTimePlanted / (max list regTimePlantedCount 1)</metric>
    <metric>avgLifespan / (max list deaths 1)</metric>
    <steppedValueSet variable="correlation-time" first="1.5" step="0.25" last="2"/>
    <enumeratedValueSet variable="range-offset">
      <value value="0.25"/>
      <value value="0.5"/>
      <value value="0.75"/>
  ...
  <experiment name="Repeat 100" repetitions="100" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>simulate</go>
    <metric>count mangroves with [species = "native"]</metric>
    <metric>count mangroves with [species = "planted"]</metric>
